...
public abstract class AbstractHero : IHero, IComparable<AbstractHero>
{
    ...
    public ICollection<IItem> Items
    {
        ...
        get
        {
            FieldInfo[] invetoryFieldInfo = this.inventory.GetType().GetFields(BindingFlags.Instance | BindingFlags.NonPublic);

            foreach (var fieldInfo in invetoryFieldInfo)
            {
                ItemAttribute itemAtrribute =
                    (ItemAttribute)fieldInfo.GetCustomAttributes(typeof(ItemAttribute), true).FirstOrDefault();

                if (itemAtrribute != null)
                {
            ...
            throw new InvalidOperationException("Hero has no item");
        }
    }
    ...
    public override string ToString()
    {
       StringBuilder sb = new StringBuilder();
        ...
        sb.AppendLine(this.Items.Count == 0 ? $"Items: None" : $"Items:");
