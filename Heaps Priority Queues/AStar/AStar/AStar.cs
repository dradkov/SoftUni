...
public class AStar
{
    ...
    public IEnumerable<Node> GetPath(Node start, Node goal)
    {
        ...
        while (open.Count > 0)
        {
            ...
            for (int i = current.Row - 1; i <= current.Row + 1; i++)
            {
                for (int j = current.Col - 1; j <= current.Col + 1; j++)
                {
                    ...
                    if (Math.Abs(Math.Abs(i - j) - Math.Abs(current.Row - current.Col)) == 1 && i >= 0 && i < this.map.GetLength(0) && j >= 0 && j < this.map.GetLength(1) && (this.map[i, j] == '-' || this.map[i, j] == '*'))
                    {
        ...
        if (parent.ContainsKey(goal))
        {
            ...
            while (!Equals(current, start) && current != null)
            {
                result.Push(current);
